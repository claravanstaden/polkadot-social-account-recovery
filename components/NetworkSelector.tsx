...
export default function NetworkSelector() {
  const { selectedNetwork, customUrls, setNetwork, setCustomUrl, getActiveWssUrl, resetCustomUrl } = useNetwork();
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (isValid) {
        ...
        setTestResult({ success: false, message: 'Connection failed. Please check the URL.' });
      }
    } catch (error) {
      setTestResult({
        success: false,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="wss-url" className="block text-sm font-medium text-gray-700 mb-2">
                  AssetHub WSS URL
                </label>
                <input
                  id="wss-url"
